...
stmt :
     ...
     | doris_statement // after call_stmt, create_procedure_stmt
     | assignment_stmt
...
set_doris_session_option :
       (GLOBAL | LOCAL | SESSION)? ident_pl EQ ident_pl // hplsql, 这里的含义是啥，什么情况会用到这个，是因为放在第一位，所以每次都用么 // hplsql会set一些局部变量，和我们 set seesion veriable冲突 // 所有的set xxx=xxx都当成了session veriables处理 //放在第一位先匹配
      ; //
