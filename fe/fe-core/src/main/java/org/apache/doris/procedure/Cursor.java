...
public class Cursor {
    ...
    Cursor.State state = Cursor.State.CLOSE;
    ...
    public void open(QueryExecutor queryExecutor, OpenStmtContext ctx) {
    ...
    public void setFetch(boolean ok) {
        if (ok) {
            state = Cursor.State.FETCHED_OK;
        } else {
            state = Cursor.State.FETCHED_NODATA;
        }
    }

    public Boolean isFound() {
        if (state == Cursor.State.OPEN || state == Cursor.State.CLOSE) {
            return null;
        }
        if (state == Cursor.State.FETCHED_OK) {
            return Boolean.TRUE;
        }
    ...
    public Boolean isNotFound() {
        ...
        if (state == Cursor.State.FETCHED_NODATA) {
            return Boolean.TRUE;
        }
        return Boolean.FALSE;
    }
    ...
    public void close() {
        if (queryResult != null) {
            queryResult.close();
            state = Cursor.State.CLOSE;
        }
    }

    public boolean isOpen() {
        return state != Cursor.State.CLOSE;
    }
}
