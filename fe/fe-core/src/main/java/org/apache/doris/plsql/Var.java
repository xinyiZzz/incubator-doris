...
public class Var {
    ...
    public static Var Null = new Var(Var.Type.NULL);

    public String name;
    public Var.Type type;
    public DataType dorisDataType = null;
    public Object value;
    ...
    public Var(Long value) {
        this.type = Var.Type.BIGINT;
        this.value = value;
    }

    public Var(BigDecimal value) {
        this.type = Var.Type.DECIMAL;
        this.value = value;
    }
    ...
    public Var(String value) {
        this.type = Var.Type.STRING;
        this.value = value;
    }

    public Var(Double value) {
        this.type = Var.Type.DOUBLE;
        this.value = value;
    }

    public Var(Date value) {
        this.type = Var.Type.DATE;
        this.value = value;
    }

    public Var(Timestamp value, int scale) {
        this.type = Var.Type.TIMESTAMP;
        this.value = value;
        this.scale = scale;
    }

    public Var(Interval value) {
        this.type = Var.Type.INTERVAL;
        this.value = value;
    }

    public Var(ArrayList<String> value) {
        this.type = Var.Type.STRINGLIST;
        this.value = value;
    }
    ...
    //     this.name = name;
    //     this.type = Var.Type.ROW;
    //     this.value = new Row(row);
    // }

    public Var(Var.Type type, String name) {
        this.type = type;
        this.name = name;
    }

    public Var(Var.Type type, Object value) {
        this.type = type;
        this.value = value;
    }

    public Var(String name, Var.Type type, Object value) {
        this.name = name;
        this.type = type;
        this.value = value;
    }
    ...
    public Var(String name, DataType dorisDataType, Integer len, Integer scale, Var def) {
        this.name = name;
    ...
    public Var(String name, DataType type, String len, String scale, Var def) {
        this(name, type, len != null ? Integer.parseInt(len) : null, scale != null ? Integer.parseInt(scale) : null,
                def);
    }
    ...
    public Var cast(String val) {
        if (!constant && type == Var.Type.STRING) {
            if (len != 0) {
                int l = val.length();
                if (l > len) {
    ...
    public Var setValue(Long val) {
        if (!constant && type == Var.Type.BIGINT) {
            value = val;
        }
        return this;
    }
    ...
    public Var setValue(Boolean val) {
        if (!constant && type == Var.Type.BOOL) {
            value = val;
        }
        return this;
    }
    ...
    public static Var.Type defineType(String type) {
        if (type == null) {
            ...
            return Var.Type.RS_LOCATOR;
        } else if (type.equalsIgnoreCase(Var.DERIVED_TYPE)) {
            return Var.Type.DERIVED_TYPE;
        } else if (type.equalsIgnoreCase(Var.Type.HPL_OBJECT.name())) {
            return Var.Type.HPL_OBJECT;
        } else if (type.equalsIgnoreCase(Var.Type.ROW.name())) {
            return Var.Type.ROW;
        }
        return Var.Type.NULL;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (type == Var.Type.BIGINT) {
            if (var.type == Var.Type.BIGINT && ((Long) value).longValue() == ((Long) var.value).longValue()) {
                return true;
            } else if (var.type == Var.Type.DECIMAL) {
                return equals((BigDecimal) var.value, (Long) value);
            }
        } else if (type == Var.Type.STRING && var.type == Var.Type.STRING && value.equals(var.value)) {
            return true;
        } else if (type == Var.Type.DECIMAL && var.type == Var.Type.DECIMAL
                && ((BigDecimal) value).compareTo((BigDecimal) var.value) == 0) {
            return true;
        } else if (type == Var.Type.DOUBLE) {
            if (var.type == Var.Type.DOUBLE && ((Double) value).compareTo((Double) var.value) == 0) {
                return true;
            } else if (var.type == Var.Type.DECIMAL
    ...
    public int compareTo(Var v) {
        if (this == v) {
            ...
            return -1;
        } else if (type == Var.Type.BIGINT && v.type == Var.Type.BIGINT) {
            return ((Long) value).compareTo((Long) v.value);
        } else if (type == Var.Type.DOUBLE && v.type == Var.Type.DECIMAL) {
            return (new BigDecimal((double) value)).compareTo((BigDecimal) v.value);
        } else if (type == Var.Type.STRING && v.type == Var.Type.STRING) {
    ...
    public boolean isTrue() {
        if (type == Var.Type.BOOL && value != null) {
    ...
    public boolean isNull() {
        if (type == Var.Type.NULL || value == null) {
            return true;
        }
        return false;
    }
