...
Status SegmentIterator::init(const StorageReadOptions& opts) {
    ...
    _remaining_vconjunct_root = _remaining_vconjunct_ctx ? _remaining_vconjunct_ctx->root() : nullptr;
    _output_slots_size = opts.output_slots_size;
...
Status SegmentIterator::_extract_remaining_expr_columns(vectorized::VExpr* expr) {
    if (expr == nullptr) {
        return Status::OK();
    }
    auto children = expr->children();
    for (int i = 0; i < children.size(); ++i) {
        RETURN_IF_ERROR(_extract_remaining_expr_columns(children[i]));
    }

    auto node_type = expr->node_type();
    if (node_type == TExprNodeType::SLOT_REF) {
        auto slot_expr = dynamic_cast<doris::vectorized::VSlotRef*>(expr);
        _is_remaining_expr_column[_schema.column_id(slot_expr->expr_name())] = true;
        _remaining_expr_columns.push_back(_schema.column_id(slot_expr->expr_name()));
    }
...
void SegmentIterator::_vec_init_lazy_materialization() {
    ...
    if (_col_predicates.empty()) {
        // LOG(INFO) << "_vec_init_lazy_materialization aaaa";
    }
    ...
    if (_lazy_materialization_read) {
        ...
        for (auto cid : pred_column_ids) {
            ...
            // LOG(INFO) << "_vec_init_lazy_materialization 0000 " << cid;
        }
    } else if (!_is_need_vec_eval &&
               !_is_need_short_eval && !_is_need_expr_eval) { // no pred exists, just read and output column
        for (int i = 0; i < _schema.num_column_ids(); i++) {
            auto cid = _schema.column_id(i);
            _first_read_column_ids.push_back(cid);
            // LOG(INFO) << "_vec_init_lazy_materialization 1111 " << cid;
        }
    } else { 
        if (!_is_need_expr_eval) {
            // pred exits, but we can eliminate lazy materialization
            // insert pred/non-pred cid to first read columns
            // _schema.column_ids().size() == pred_column_ids.size()
            ...
            for (int i = 0; i < _schema.num_column_ids(); i++) {
                ...
                if (pred_id_set.find(cid) != pred_id_set.end()) {
                    _first_read_column_ids.push_back(cid);
                    // LOG(INFO) << "_vec_init_lazy_materialization 2222 aaa " << cid;
                    ...
                    _is_pred_column[cid] = true;
                    // LOG(INFO) << "_vec_init_lazy_materialization 2222 bbb " << cid;
                }
            }
        } else if (!_is_need_vec_eval && !_is_need_short_eval) {
            for (auto cid : _remaining_expr_columns) {
                _first_read_column_ids.push_back(cid);
                // LOG(INFO) << "_vec_init_lazy_materialization 3333 " << cid;
            }
        } else {
            // _is_need_expr_eval = true, 意味着肯定有column不在_is_pred_column中，
            // 那如果_is_need_vec_eval或_is_need_short_eval有一个为true，_lazy_materialization_read不可能等于false
            // 这些逻辑都该重构下。。。。。。
            CHECK(false);
        }
    }
...
Status SegmentIterator::_read_columns_by_index(uint32_t nrows_read_limit, uint32_t& nrows_read,
    ...
    std::stringstream ss;
    for (auto it = _first_read_column_ids.begin(); it != _first_read_column_ids.end(); it++)    {
        if (it != _first_read_column_ids.begin()) {
            ss << ",";
        }
        ss << *it;
    }
    // LOG(INFO) << "_read_columns_by_index 000 " << ss.str();
    do {
        uint32_t range_from;
        uint32_t range_to;
        bool has_next_range =
        ...
        if (set_block_rowid) {
            ...
            // LOG(INFO) << "_read_columns_by_index " << range_from << ", " << range_to << ", " << _block_rowids.size();
...
Status SegmentIterator::next_batch(vectorized::Block* block) {
    // LOG(INFO) << "next_batch; " << block->get_by_position(1).column->size() << " , block_rows " << block->each_col_size() << " , rows " << block->rows();
    ...
    if (UNLIKELY(!_inited)) {
        ...
        for (size_t i = 0; i < _schema.num_column_ids(); i++) {
            ...
            if (_is_pred_column[cid]) {
                ...
                // if i >= block->columns means the column and1not the pred_column means `column i` is
    ...
    if (!_is_need_vec_eval && !_is_need_short_eval && !_is_need_expr_eval) {
        ...
        if (_is_need_vec_eval || _is_need_short_eval) {
            // LOG(INFO) << "_is_need_vec_eval _is_need_short_eval 111";
            ...
            if (selected_size > 0) {
                // LOG(INFO) << "4.3 output short circuit and predicate column " << selected_size;
                RETURN_IF_ERROR(_output_column_by_sel_idx(block, _first_read_column_ids, sel_rowid_idx,
                                            selected_size));
                // if (_is_need_expr_eval) {
                //     for (auto cid : _remaining_expr_columns) { // _first_read_column_ids = _remaining_expr_columns
                //         auto block_cid = _schema_block_id_map[cid];
                //         RETURN_IF_ERROR(block->copy_column_data_to_block(_current_return_columns[cid].get(),
                //                                                         sel_rowid_idx, selected_size, block_cid,
                //                                                         _opts.block_row_max));
                //     }
                // }

                if (_is_need_expr_eval) {
                    // LOG(INFO) << "_is_need_vec_eval _is_need_short_eval 222";
                    // read expr eval column 
                    // tmp_block = block->clone_empty(); // tmp_block 作为 成员

                    std::vector<uint32_t> columns_to_filter;
                    for (auto cid : _remaining_expr_columns) {
                        auto loc = _schema_block_id_map[cid];
                        columns_to_filter.push_back(loc);
                        DCHECK(loc < block->columns());
                        // LOG(INFO) << "_is_need_vec_eval _is_need_short_eval 777 " << cid << ", " << _remaining_expr_columns.size();
                        if (std::find(_first_read_column_ids.begin(), _first_read_column_ids.end(), cid) == _first_read_column_ids.end()) {
                            _second_read_column_ids.push_back(cid);
                            // LOG(INFO) << "_is_need_vec_eval _is_need_short_eval 6666 " << cid;
                        }
                    }
                    ...
                    RETURN_IF_ERROR(_read_columns_by_rowids(_second_read_column_ids, _block_rowids,
                                                        sel_rowid_idx, selected_size));
                    for (auto cid : _second_read_column_ids) {
                        auto loc = _schema_block_id_map[cid];
                        // LOG(INFO) << "_is_need_vec_eval _is_need_short_eval 333 " << _second_read_column_ids.size() << ", " << cid << ", " << loc << ", " << block->get_by_position(loc).column->size() << " selected_size, " << selected_size << " , block_rows " << block->each_col_size();
                        // LOG(INFO) << "_is_need_vec_eval _is_need_short_eval 444 " << _current_return_columns[cid]->size();
                        block->replace_by_position(loc, std::move(_current_return_columns[cid]));
                        // LOG(INFO) << "_is_need_vec_eval _is_need_short_eval 555 " << loc << ", " << block->get_by_position(loc).column->size() << " selected_size, " << selected_size << " , block_rows " << block->each_col_size();
                    }
                    
                    // LOG(INFO) << "_is_need_vec_eval _is_need_short_eval 555 " << " selected_size, " << selected_size << " , block_rows " << block->each_col_size();
                    selected_size = _execute_remaining_expr(sel_rowid_idx, selected_size, block, columns_to_filter);
                }
            }
        } else if (_is_need_expr_eval) {
            // LOG(INFO) << "_is_need_expr_eval 111";
            for (uint32_t i = 0; i < selected_size; ++i) {
                sel_rowid_idx[i] = i;
            }

            // tmp_block = block->clone_empty();
            std::vector<uint32_t> columns_to_filter;
            if (selected_size > 0) {
                // RETURN_IF_ERROR(_output_column_by_sel_idx(&tmp_block, _first_read_column_ids, sel_rowid_idx,
                //                                         selected_size));
                for (auto cid : _remaining_expr_columns) { // _first_read_column_ids = _remaining_expr_columns
                    auto loc = _schema_block_id_map[cid];
                    columns_to_filter.push_back(loc);
        ...
        if (_non_predicate_columns.empty()) {
            // LOG(INFO) << "_non_predicate_columns selected_size " << selected_size;
            ...
            //     //     for (auto cid : _remaining_expr_columns) { // _first_read_column_ids = _remaining_expr_columns
            //     //         auto block_cid = _schema_block_id_map[cid];
            //     //         RETURN_IF_ERROR(block->copy_column_data_to_block(tmp_block.get_by_position(block_cid).column.get(),
            //     //                                                         sel_rowid_idx, selected_size, block_cid,
            //     //                                                         _opts.block_row_max));
            //     //     }
            //     // }
        ...
        // LOG(INFO) << "step3: read non_predicate column selected_size " << selected_size;
    ...
    // LOG(INFO) << "_update_max_row(block); " << block->get_by_position(1).column->size() << " , block_rows " << block->each_col_size() << " , rows " << block->rows();
...
uint16_t SegmentIterator::_execute_remaining_expr(uint16_t* sel_rowid_idx, uint16_t selected_size, vectorized::Block* block, const std::vector<uint32_t>& columns_to_filter) {
    if (_remaining_vconjunct_root != nullptr && block->rows() != 0) {
        size_t prev_columns = block->columns();
        // LOG(INFO) << "selected_size 0000 ";
        int result_column_id = -1;
        // LOG(INFO) << "SegmentIterator::_execute_remaining_expr aaa " << " selected_size, " << selected_size << " , block_rows " << block->each_col_size();
        RETURN_IF_ERROR(_remaining_vconjunct_ctx->execute(block, &result_column_id));
        // LOG(INFO) << "SegmentIterator::_execute_remaining_expr bbb " << " selected_size, " << selected_size << " , block_rows " << block->each_col_size() << ", result_column_id " << result_column_id;
        vectorized::ColumnPtr filter_column = block->get_by_position(result_column_id).column;
        if (auto* nullable_column = vectorized::check_and_get_column<vectorized::ColumnNullable>(*filter_column)) {
            // LOG(INFO) << "selected_size aaaa " << nullable_column->byte_size();
            ...
            vectorized::ColumnUInt8* concrete_column = typeid_cast<vectorized::ColumnUInt8*>(mutable_holder.get());
            if (!concrete_column) {
            ...
            vectorized::Block::filter_block_internal(block, columns_to_filter, filter);
        } else if (auto* const_column = vectorized::check_and_get_column<vectorized::ColumnConst>(*filter_column)) {
            // LOG(INFO) << "selected_size bbbb " << const_column->byte_size();
            bool ret = const_column->get_bool(0);
            if (!ret) {
                selected_size = 0;
                for (auto& col : columns_to_filter) { // 或许不用clear？
                    std::move(*block->get_by_position(col).column).assume_mutable()->clear();
                }
            }
        } else {
            // LOG(INFO) << "selected_size cccc ";
            const vectorized::IColumn::Filter& filter =
                assert_cast<const doris::vectorized::ColumnVector<vectorized::UInt8>&>(*filter_column)
                        .get_data();
            // LOG(INFO) << "SegmentIterator::_execute_remaining_expr 000 " << " selected_size, " << selected_size << " , block_rows " << block->each_col_size();
            selected_size = _evaluate_remaining_expr_filter(sel_rowid_idx, selected_size, filter);
            vectorized::Block::filter_block_internal(block, columns_to_filter, filter);
            std::stringstream ss;
            ...
            // LOG(INFO) << "SegmentIterator::_execute_remaining_expr 111 " << " selected_size, " << selected_size << " , block_rows " << block->each_col_size() << ", " << ss.str();
        }
        for (int i = block->columns() - 1; i >= prev_columns; --i) {
            block->erase(i);
        }
        // LOG(INFO) << "SegmentIterator::_execute_remaining_expr 222 " << " selected_size, " << selected_size << " , block_rows " << block->each_col_size();
    }
    return selected_size;
}

uint16_t SegmentIterator::_evaluate_remaining_expr_filter(uint16_t* sel_rowid_idx, uint16_t selected_size, const vectorized::IColumn::Filter& filter) {
    ...
    if (count == 0) {
        ...
        // LOG(INFO) << "SegmentIterator::next_batch selected_size 000 " << selected_size;
        selected_size = new_size;
        // LOG(INFO) << "SegmentIterator::next_batch selected_size 111 " << selected_size;
        return selected_size;
    }
}
