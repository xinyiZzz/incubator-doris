...
Status SegmentIterator::init(const StorageReadOptions& opts) {
    ...
    _remaining_vconjunct = opts.remaining_vconjunct_root;
    _remaining_vconjunct_root = _remaining_vconjunct ? _remaining_vconjunct->root() : nullptr;
...
Status SegmentIterator::_extract_expr_filter_columns(vectorized::VExpr* expr) {
    if (expr == nullptr) {
        return Status::OK();
    }
    auto children = expr->children();
    for (int i = 0; i < children.size(); ++i) {
        RETURN_IF_ERROR(_extract_expr_filter_columns(children[i]));
    }

    auto node_type = expr->node_type();
    if (node_type == TExprNodeType::SLOT_REF) {
        auto slot_expr = dynamic_cast<doris::vectorized::VSlotRef*>(expr);
        _is_pred_column_used_expr[_schema.column_id(slot_expr->expr_name())] = true;
        _expr_columns.push_back(_schema.column_id(slot_expr->expr_name()));
    }
...
void SegmentIterator::_vec_init_lazy_materialization() {
    ...
    _is_pred_column_used_expr.resize(_schema.columns().size(), false);
    _extract_expr_filter_columns(_remaining_vconjunct_root);
    if (!_expr_columns.empty()) {
    ...
    if (_schema.column_ids().size() > pred_column_ids.size()) {
        for (auto cid : _schema.column_ids()) {
            if (!_is_pred_column[cid]) {
                if (!_is_pred_column_used_expr[cid]) {
...
void SegmentIterator::_init_current_block(
    ...
    for (size_t i = 0; i < _schema.num_column_ids(); i++) {
        ...
        if (_is_pred_column[cid] ||
            i >= block->columns() || _is_pred_column_used_expr[cid]) { //todo(wb) maybe we can release it after output block
...
Status SegmentIterator::next_batch(vectorized::Block* block) {
    ...
    if (UNLIKELY(!_inited)) {
        ...
        for (size_t i = 0; i < _schema.num_column_ids(); i++) {
            auto cid = _schema.column_id(i);
            auto column_desc = _schema.column(cid);
            if (_is_pred_column[cid] || _is_pred_column_used_expr[cid]) {
    ...
    if (_current_batch_rows_read == 0) {
        for (int i = 0; i < block->columns(); i++) {
            ...
            if (!_is_pred_column[cid] && !_is_pred_column_used_expr[cid]) { // non-predicate
    ...
    if (!_is_need_vec_eval && !_is_need_short_eval && !_is_need_expr_eval) {
        ...
        if (_is_need_vec_eval || _is_need_short_eval) {
            LOG(INFO) << "_is_need_vec_eval _is_need_short_eval";
            ...
            if (_is_need_expr_eval) {
                ...
                RETURN_IF_ERROR(_read_columns_by_rowids(_expr_columns, _block_rowids,
                ...
                for (auto cid : _expr_columns) {
                    ...
                    // if loc > block->columns() means the column is delete column and should
                    // not output by block, so just skip the column.
                    if (loc < tmp_block.columns()) {
                        tmp_block.replace_by_position(loc, std::move(_current_return_columns[cid]));
                    }
                }

                selected_size = _execute_expr(sel_rowid_idx, selected_size, tmp_block);
            }
...
uint16_t SegmentIterator::_execute_expr(uint16_t* sel_rowid_idx, uint16_t selected_size, vectorized::Block& block) {
    if (_remaining_vconjunct_root != nullptr && block.rows() != 0) {
        int result_column_id = -1;
        RETURN_IF_ERROR(_remaining_vconjunct->execute(&block, &result_column_id));
        ...
        if (auto* nullable_column = vectorized::check_and_get_column<vectorized::ColumnNullable>(*filter_column)) {
            ...
            selected_size = _expr_filter(sel_rowid_idx, selected_size, filter);
...
uint16_t SegmentIterator::_expr_filter(uint16_t* sel_rowid_idx, uint16_t selected_size, const vectorized::IColumn::Filter& filter) {
