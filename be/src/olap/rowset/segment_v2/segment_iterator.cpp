...
Status SegmentIterator::init(const StorageReadOptions& opts) {
    ...
    _enable_remaining_expr_pushdown = opts.enable_remaining_expr_pushdown;
...
Status SegmentIterator::_extract_remaining_expr_columns(vectorized::VExpr* expr) {
    ...
    if (node_type == TExprNodeType::SLOT_REF) {
        ...
        _is_remaining_expr_column[_schema.column_id(slot_expr->column_id())] = true;
        _remaining_expr_columns.insert(_schema.column_id(slot_expr->column_id()));
        LOG(INFO) << "_extract_remaining_expr_columns 111 " << slot_expr->slot_id() << ", " << _schema.column_id(slot_expr->slot_id()) << ", " << slot_expr->column_id() <<  ", " << _schema.column_id(slot_expr->column_id());
    }
...
Status SegmentIterator::_init_return_column_iterators() {
    ...
    for (auto cid : _schema.column_ids()) {
        LOG(INFO) << "_init_return_column_iterators " << cid << ", " << _opts.tablet_schema->column(cid).unique_id();
        int32_t unique_id = _opts.tablet_schema->column(cid).unique_id();
...
Status SegmentIterator::_seek_columns(const std::vector<ColumnId>& column_ids, rowid_t pos) {
    for (auto cid : column_ids) {
        LOG(INFO) << "SegmentIterator::_seek_columns " << cid << ", _schema.unique_id " << _schema.unique_id(cid) << ", " << _column_iterators.size();
...
void SegmentIterator::_vec_init_lazy_materialization() {
        ...
        if (!_remaining_expr_columns.empty()) {
            ...
            for (auto cid : _schema.column_ids()) {
                ...
                if (_is_remaining_expr_column[cid] || _is_pred_column[cid]) {
                    ...
                    // LOG(INFO) << "_schema_block_id_map 4444 " << cid << ", " << loc;
    ...
    if (_schema.column_ids().size() > pred_column_ids.size()) {
        for (auto cid : _schema.column_ids()) {
            // LOG(INFO) << "_is_remaining_expr_column 000 " << cid;
            if (!_is_pred_column[cid]) {
                if (!_is_remaining_expr_column[cid]) {
                    _non_predicate_columns.push_back(cid);
                    // LOG(INFO) << "_is_remaining_expr_column 111 " << cid;
                } else {
                    _second_read_column_ids.push_back(cid);
                    // LOG(INFO) << "_is_remaining_expr_column 222 " << cid;
                }
    ...
    if (_lazy_materialization_read) {
        ...
        for (auto cid : pred_column_ids) {
            // LOG(INFO) << "_lazy_materialization_read 111 " << cid;
            // LOG(INFO) << "_first_read_column_ids 33333 cid:" << cid << ", i:";
        ...
        for (int i = 0; i < _schema.num_column_ids(); i++) {
            auto cid = _schema.column_id(i);
            _first_read_column_ids.push_back(cid);
            // LOG(INFO) << "_first_read_column_ids 222 cid:" << cid << ", i:" << i;
        }
    } else {
        if (!_is_need_expr_eval) {
            ...
            for (int i = 0; i < _schema.num_column_ids(); i++) {
                auto cid = _schema.column_id(i);
                // LOG(INFO) << "_first_read_column_ids 5554444 cid:" << cid << ", i:" << i;
            ...
            for (auto cid : _remaining_expr_columns) {
                // LOG(INFO) << "_first_read_column_ids 1111 cid:" << cid << ", i:";
                _first_read_column_ids.push_back(cid);
            }
        }
    }
...
void SegmentIterator::_output_non_pred_columns(vectorized::Block* block) {
    ...
    for (auto cid : _non_predicate_columns) {
        ...
        if (loc < block->columns()) {
            ...
            // LOG(INFO) << "SegmentIterator::_output_non_pred_columns 0000";
        }
    }
}
...
Status SegmentIterator::next_batch(vectorized::Block* block) {
    ...
    if (!_is_need_vec_eval && !_is_need_short_eval && !_is_need_expr_eval) {
        // LOG(INFO) << "SegmentIterator::next_batch _is_need_vec_eval 000 ";
        ...
        if (_is_need_vec_eval || _is_need_short_eval) {
            ...
            if (selected_size > 0) {
                ...
                if (_is_need_expr_eval) {
                    if (!_second_read_column_ids.empty()) {
                        ...
                        RETURN_IF_ERROR(_read_columns_by_rowids(_second_read_column_ids,
                                                                _block_rowids, sel_rowid_idx,
                                                                selected_size));
                        ...
                        block->set_effective_col(_schema_block_id_map[_second_read_column_ids[0]]);
                    }
                    // LOG(INFO) << "SegmentIterator::next_batch _is_need_expr_eval 222 " << selected_size;

                    selected_size = _execute_remaining_expr(sel_rowid_idx, selected_size, block);
            ...
            //LOG(INFO) << "SegmentIterator::next_batch _is_need_expr_eval 000 " << selected_size << ", " << _first_read_column_ids.size();
            // for (int i = 0; i < _first_read_column_ids.size(); i++) {
            //     //LOG(INFO) << "SegmentIterator::next_batch _is_need_expr_eval xxx " << i << ", " << _first_read_column_ids.size();
            // }
            DCHECK(!_first_read_column_ids.empty());
            for (auto cid : _first_read_column_ids) {
                auto loc = _schema_block_id_map[cid];
                //LOG(INFO) << "SegmentIterator::next_batch _is_need_expr_eval 111 " << cid;
                block->replace_by_position(loc, std::move(_current_return_columns[cid]));
            }
            block->set_effective_col(_schema_block_id_map[_first_read_column_ids[0]]);
            //LOG(INFO) << "SegmentIterator::next_batch _is_need_expr_eval 333 " << block->each_col_size();
        ...
        if (UNLIKELY(_opts.record_rowids)) {
            // LOG(INFO) << "if (UNLIKELY(_opts.record_rowids)) {)";
        ...
        if (_non_predicate_columns.empty()) {
            ...
            // LOG(INFO) << "SegmentIterator::next_batch _non_predicate_columns.empty() block: " << block->each_col_size();
            return Status::OK();
        }

        // step4: read non_predicate column
        if (selected_size > 0) {
            // LOG(INFO) << "SegmentIterator::next_batch selected_size > 0";
...
uint16_t SegmentIterator::_execute_remaining_expr(uint16_t* sel_rowid_idx, uint16_t selected_size,
    ...
    if (auto* nullable_column =
        ...
        // std::stringstream ss;
        // for (auto it = _columns_to_filter.begin(); it != _columns_to_filter.end(); it++) {
        //     if (it != _columns_to_filter.begin()) {
        //         ss << ",";
        //     }
        //     ss << *it;
        // }
        // LOG(INFO) << "_execute_remaining_expr 333 " << block->each_col_size() << ", " << ss.str() << ", " << selected_size;
        ...
        selected_size = _evaluate_remaining_expr_filter(sel_rowid_idx, selected_size, filter);
        vectorized::Block::filter_block_internal(block, _columns_to_filter, filter);
        // LOG(INFO) << "_execute_remaining_expr 333 444 " << block->each_col_size() << ", " << ss.str() << ", " << selected_size;
    }

    vectorized::Block::erase_useless_column(block, prev_columns);
    return selected_size;
}

uint16_t SegmentIterator::_evaluate_remaining_expr_filter(
        uint16_t* sel_rowid_idx, uint16_t selected_size,
        const vectorized::IColumn::Filter& filter) {
    size_t count = filter.size() - simd::count_zero_num((int8_t*)filter.data(), filter.size());
    if (count == 0) {
        // LOG(INFO) << "_evaluate_remaining_expr_filter 000;";
        return 0;
    } else {
        // LOG(INFO) << "_evaluate_remaining_expr_filter 111;";
