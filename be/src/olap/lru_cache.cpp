...
Cache::Handle* LRUCache::insert(const CacheKey& key, uint32_t hash, void* value, size_t charge,
                                void (*deleter)(const CacheKey& key, void* value), MemTrackerLimiter* tracker,
    ...
    tls_ctx()->_thread_mem_tracker_mgr->limiter_mem_tracker()->transfer_to(tracker, e->total_size);
...
ShardedLRUCache::ShardedLRUCache(const std::string& name, size_t total_capacity, LRUCacheType type,
        ...
        : _name(name),
          ...
          _shards(nullptr),
          _last_id(1) {
    _mem_tracker = std::make_unique<MemTrackerLimiter>(name);
...
Cache::Handle* ShardedLRUCache::insert(const CacheKey& key, void* value, size_t charge,
    ...
    return _shards[_shard(hash)]->insert(key, hash, value, charge, deleter, _mem_tracker.get(), priority);
}
