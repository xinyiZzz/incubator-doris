...
{
    _mem_tracker = std::make_unique<MemTrackerLimiter>(-1, "DiskIO");
...
Status DiskIoMgr::init(const int64_t mem_limit) {
    ...
    MemTrackerLimiter::get_process_tracker_limiter()->add_gc_function(
...
char* DiskIoMgr::get_free_buffer(int64_t* buffer_size) {
    ...
    *buffer_size = (1 << idx) * _min_buffer_size;
    ...
    if (_free_buffers[idx].empty()) {
        ...
        _mem_tracker->transfer_to(tls_ctx()->_thread_mem_tracker_mgr->limiter_mem_tracker(),
                                  *buffer_size);
...
void DiskIoMgr::gc_io_buffers(int64_t bytes_to_free) {
    ...
    _mem_tracker->transfer_to(tls_ctx()->_thread_mem_tracker_mgr->limiter_mem_tracker(), bytes_freed);
}
...
void DiskIoMgr::return_free_buffer(char* buffer, int64_t buffer_size) {
    ...
    if (!config::disable_mem_pools && _free_buffers[idx].size() < config::max_free_io_buffers) {
        ...
        tls_ctx()->_thread_mem_tracker_mgr->limiter_mem_tracker()->transfer_to(_mem_tracker.get(), buffer_size);
...
void DiskIoMgr::work_loop(DiskQueue* disk_queue, MemTrackerLimiter* mem_tracker) {
    ...
    // SCOPED_THREAD_ATTACH_LIMITER_MEM_TRACKER(ThreadContext::TaskType::STORAGE, mem_tracker);
    while (!_shut_down) {
