...
Status LoadChannelMgr::open(const PTabletWriterOpenRequest& params) {
    ...
    std::shared_ptr<LoadChannel> channel;
    {
        ...
        if (it != _load_channels.end()) {
            ...
            std::unique_ptr<MemTrackerLimiter> channel_mem_tracker = std::make_unique<MemTrackerLimiter>(channel_mem_limit,
                                               fmt::format("LoadChannel#senderIp={}#loadID={}",
                                                           params.sender_ip(), load_id.to_string()), _mem_tracker);
            channel.reset(new LoadChannel(load_id, std::move(channel_mem_tracker), channel_timeout_s,
