...
MemPool::MemPool(const std::string& label)
        : current_chunk_idx_(-1),
          ...
          total_reserved_bytes_(0),
          peak_allocated_bytes_(0) {
    _mem_tracker = std::make_unique<MemTracker>(fmt::format("MemPool:{}", label));
    }
...
Status MemPool::find_chunk(size_t min_size, bool check_limits) {
    ...
    if (check_limits && !tls_ctx()->_thread_mem_tracker_mgr->limiter_mem_tracker()->check_limit(chunk_size)) {
        return Status::MemoryAllocFailed("MemPool find new chunk {} bytes faild, exceed limit", chunk_size);
    }

    // Allocate a new chunk. Return early if allocate fails.
    Chunk chunk;
    RETURN_IF_ERROR(
            ChunkAllocator::instance()->allocate(chunk_size, &chunk));
