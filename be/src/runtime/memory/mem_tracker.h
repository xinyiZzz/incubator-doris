...
public:
    ...
    MemTracker(const std::string& label, RuntimeProfile* profile = nullptr, MemTrackerLimiter* parent = nullptr);
...
public:
    ...
    bool check_limit(int64_t limit, int64_t bytes) const { return limit >= 0 && limit < consumption() + bytes; }
