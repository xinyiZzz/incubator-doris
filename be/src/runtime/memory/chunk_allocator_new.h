...
class Suballocator {
 public:
  ...
  Suballocator(int64_t min_buffer_len);
  ...
  bool Allocate(int64_t bytes, std::unique_ptr<Suballocation>* result);
 ...
 private:
  ...
  bool AllocateBuffer(int64_t bytes, std::unique_ptr<Suballocation>* result);
  ...
  /// fails.
  bool SplitToSize(std::unique_ptr<Suballocation> node, int64_t target_bytes,
