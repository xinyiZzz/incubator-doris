...
MemTrackerObserve* MemTrackerObserve::create_tracker(const std::string& label, RuntimeProfile* profile) {
    ...
    // MemTrackerLimiter* parent = tls_ctx()->_thread_mem_tracker_mgr->mem_tracker();
    MemTrackerLimiter* parent = MemTrackerLimiter::get_process_tracker();
    DCHECK(parent);
...
MemTrackerObserve::~MemTrackerObserve() {
    if (config::memory_leak_detection) MemTrackerObserve::memory_leak_check(this);
    if (parent()) {
