...
void new_hook(const void* ptr, size_t size) {
    if (doris::thread_local_ctx._init) {
        ...
        doris::MemTrackerLimiter::get_process_tracker()->consume(tc_nallocx(size, 0));
    }
}
...
void delete_hook(const void* ptr) {
    if (doris::thread_local_ctx._init) {
        ...
        doris::MemTrackerLimiter::get_process_tracker()->release(tc_malloc_size(const_cast<void*>(ptr)));
    }
}
