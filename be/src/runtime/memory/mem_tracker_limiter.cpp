...
static MemTrackerLimiter* process_tracker_limiter;
static GoogleOnceType process_tracker_limiter_once = GOOGLE_ONCE_INIT;
...
void MemTrackerLimiter::create_process_tracker_limiter() {
    process_tracker_limiter = new MemTrackerLimiter(-1, "Process", nullptr, nullptr);
}

MemTrackerLimiter* MemTrackerLimiter::get_process_tracker_limiter() {
    GoogleOnceInit(&process_tracker_limiter_once, &MemTrackerLimiter::create_process_tracker_limiter);
    return process_tracker_limiter;
}

void MemTrackerLimiter::make_snapshot(std::vector<MemTracker::Snapshot>* snapshots, size_t cur_level,
                                      size_t upper_level) const {
    (*snapshots).emplace_back(make_snapshot(cur_level));
    if (cur_level < upper_level) {
        ...
        }
        {
            ...
            for (const auto& child : _child_trackers) {
                child->make_snapshot(cur_level + 1);
            }
        }
    }
}
...
std::string MemTrackerLimiter::log_usage(int max_recursive_depth, int64_t* logged_consumption) {
    ...
    {
        std::lock_guard<std::mutex> l(_child_tracker_limiter_lock);
        child_trackers_usage = log_usage(max_recursive_depth - 1, _child_tracker_limiters, &child_consumption);
    }
    {
        std::lock_guard<std::mutex> l(_child_tracker_lock);
        for (const auto& child : _child_trackers) {
...
Status MemTrackerLimiter::mem_limit_exceeded(RuntimeState* state, const std::string& details,
    ...
    MemTrackerLimiter* process_tracker = MemTrackerLimiter::get_process_tracker_limiter();
    std::string detail =
