    ...
    ~ChunkArena() {
        for (int i = 0; i < 64; ++i) {
            ...
            for (auto ptr : _chunk_lists[i]) {
                STOP_THREAD_CONSUME_NO_LIMITER_TRACKER();
...
ChunkAllocator::ChunkAllocator(size_t reserve_limit)
    ...
    _mem_tracker = new MemTrackerLimiter(-1, "ChunkAllocator");
...
Status ChunkAllocator::allocate(size_t size, Chunk* chunk) {
    // In advance, transfer the memory ownership of allocate from ChunkAllocator::tracker to the tls tracker.
    // Next, if the allocate is successful, it will exit normally;
    // if the allocate fails, return this part of the memory to the tls tracker.
     _mem_tracker->transfer_to(tls_ctx()->_thread_mem_tracker_mgr->limiter_mem_tracker(), size);
    SCOPED_THREAD_ATTACH_LIMITER_MEM_TRACKER(_mem_tracker);
    ...
    if (chunk->data == nullptr) {
        ...
        tls_ctx()->_thread_mem_tracker_mgr->limiter_mem_tracker()->transfer_to(_mem_tracker.get(), size);
