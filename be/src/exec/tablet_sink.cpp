...
void NodeChannel::try_send_batch(RuntimeState* state) {
    ...
    if (row_batch->num_rows() > 0) {
        ...
        if (config::brpc_request_embed_attachment_send_by_http && row_batch->total_byte_size() > MIN_HTTP_BRPC_SIZE) {
    ...
    if (_tuple_data_buffer_ptr != nullptr && _tuple_data_buffer.size() != 0 &&
        ...
        request_embed_attachment_contain_row_batch<PTabletWriterAddBatchRequest,
