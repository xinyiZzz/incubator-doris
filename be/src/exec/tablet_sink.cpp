...
NodeChannel::NodeChannel(OlapTableSink* parent, IndexChannel* index_channel, int64_t node_id)
    ...
    _node_channel_tracker = std::make_unique<MemTracker>(fmt::format("NodeChannel:indexID={}:threadId={}",
...
Status NodeChannel::init(RuntimeState* state) {
    SCOPED_THREAD_CONSUME_MEM_TRACKER(_node_channel_tracker.get());
...
Status OlapTableSink::prepare(RuntimeState* state) {
    ...
    _mem_tracker = std::make_unique<MemTracker>("OlapTableSink:" + std::to_string(state->load_job_id()));
    ...
    if (!_is_vectorized) {
        ...
        RETURN_IF_ERROR(
                Expr::prepare(_output_expr_ctxs, state, _input_row_desc));
    }
