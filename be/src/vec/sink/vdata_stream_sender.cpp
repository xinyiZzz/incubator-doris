...
Status BlockSerializer::serialize_block(const Block* src, PBlock* dest, size_t num_receivers) {
    ...
    _parent->get_resource_stats_ptr()->io_stats->shuffle_send_bytes_counter_.add(compressed_bytes * num_receivers);
    _parent->get_resource_stats_ptr()->io_stats->shuffle_send_rows_counter_.add(src->rows() * num_receivers);
