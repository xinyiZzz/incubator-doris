...
void PInternalServiceImpl::transmit_data(google::protobuf::RpcController* cntl_base,
    ...
    SCOPED_THREAD_SWITCH_BTHREAD_TLS();
...
void PInternalServiceImpl::_transmit_data(google::protobuf::RpcController* cntl_base,
    ...
    if (request->has_query_id()) {
        ...
        transmit_tracker = std::make_unique<MemTrackerLimiter>(-1, fmt::format("QueryTransmit#queryId={}", query_id), _exec_env->task_pool_mem_tracker_registry()->get_task_mem_tracker(query_id));
    } else {
        query_id = "unkown_transmit_data";
        transmit_tracker = std::make_unique<MemTrackerLimiter>(-1, "unkown_transmit_data");
    }
    SCOPED_THREAD_ATTACH_LIMITER_MEM_TRACKER(transmit_tracker.get(), ThreadContext::TaskType::QUERY, query_id, finst_id);
