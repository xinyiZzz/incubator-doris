...
inline void request_embed_attachment(Params* brpc_request, const std::string& data,
                                     Closure* closure) {
    LOG(WARNING) << "http_brpc, request_embed_attachment start, data.size: " << data.size() << std::endl;
    ...
    digest1.digest();
    LOG(WARNING) << "http_brpc, send attachment.size(): " << closure->cntl.request_attachment().size()
...
inline void attachment_extract_request(const Params* brpc_request, brpc::Controller* cntl,
    ...
    LOG(WARNING) << "http_brpc, attachment_extract_request start, io_buf.size: " << io_buf.size() << std::endl;
    ...
    digest1.digest();
    LOG(WARNING) << "http_brpc, revice io_buf.size(): " << io_buf.size() << ", req_str.size: " << req_str_size
